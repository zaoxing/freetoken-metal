...
def test_metadata_is_populated(model: ftm.Model) -> None:
...
def test_metadata_only_load_skips_allocation() -> None:
    params = ftm.ModelParams()
    params.no_alloc = True
    meta_only = ftm.Model(MODEL_PATH, params)
    assert meta_only.n_layer > 0
    assert meta_only.size_bytes > 0


def test_tokenize_roundtrips(model: ftm.Model) -> None:
...
def test_generation_produces_text(model: ftm.Model) -> None:
    ctx_params = ftm.ContextParams()
    ctx_params.n_ctx = 512
    ctx = ftm.Context(model, ctx_params, ftm.SamplerParams())

    pieces = list(ftm.generate(model, ctx, "The capital of France is", max_tokens=8))
    assert pieces, "generation yielded nothing"
    assert "".join(pieces).strip(), "generation yielded only whitespace"


def test_greedy_is_deterministic(model: ftm.Model) -> None:
    def run() -> str:
        ctx_params = ftm.ContextParams()
        ctx_params.n_ctx = 512
        ctx = ftm.Context(model, ctx_params, ftm.SamplerParams())  # temp=0 -> greedy
        return "".join(ftm.generate(model, ctx, "Count: 1 2 3", max_tokens=8))

    assert run() == run()


def test_effective_geometry_differs_from_request(model: ftm.Model) -> None:
...
def test_oversized_batch_raises_instead_of_aborting(model: ftm.Model) -> None:
...
def test_prefill_longer_than_n_batch_is_chunked(model: ftm.Model) -> None:
    ctx_params = ftm.ContextParams()
    ...
    pieces = list(ftm.generate(model, ctx, prompt, max_tokens=4))
    assert pieces


def test_prompt_longer_than_context_is_rejected(model: ftm.Model) -> None:
    ctx_params = ftm.ContextParams()
    ...
    with pytest.raises(ValueError, match="context is"):
        list(ftm.generate(model, ctx, huge, max_tokens=4))
