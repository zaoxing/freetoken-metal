...
Needs FTM_TEST_MODEL like the other engine tests; skips without it. The
...
pytestmark = pytest.mark.skipif(
    ...
    reason="set FTM_TEST_MODEL to a .gguf path to run prefix-cache tests",
)
...
def test_disabled_by_default(model: ftm.Model) -> None:
...
def test_shared_prefix_forks_identical(model: ftm.Model) -> None:
...
def test_exact_rerun_skips_prefill(model: ftm.Model) -> None:
    cached = MetalEngine(model, cache_config())
    run(cached, PROMPT_A)
...
def test_below_threshold_no_fork(model: ftm.Model) -> None:
    cached = MetalEngine(model, cache_config())
...
def test_eviction_bounded(model: ftm.Model) -> None:
    cached = MetalEngine(
...
def test_single_slot_eviction_path(model: ftm.Model) -> None:
...
def test_cancel_hygiene(model: ftm.Model) -> None:
    cached = MetalEngine(model, cache_config())
    run(cached, PROMPT_A)
...
def test_stop_filter_agrees_under_fork(model: ftm.Model) -> None:
...
def test_forks_are_deterministic(model: ftm.Model) -> None:
...
def test_config_validation(model: ftm.Model) -> None:
...
def test_hybrid_arch_rejected_at_construction(model: ftm.Model) -> None:
    ...
    from freetoken_mac.engine.metal_engine import check_prefix_cache_arch
...
def test_hybrid_target_refused(
    ...
    real_meta = ftm.Model.meta_val

    def fake_meta(self: ftm.Model, key: str) -> str:
        if key == "general.architecture":
            return "qwen35"
        return real_meta(self, key)

    monkeypatch.setattr(ftm.Model, "meta_val", fake_meta)
    with pytest.raises(ValueError, match="hybrid"):
...
def test_fork_failure_falls_back_to_plain(
    ...
    run(cached, PROMPT_A)
    assert len(cached._pins) == 1
    real_rm = ftm.Context.memory_seq_rm

    def flaky_rm(self: ftm.Context, seq_id: int, p0: int = -1, p1: int = -1) -> bool:
        if p0 is not None and p0 >= 0:
            return False
        return real_rm(self, seq_id, p0, p1)

    monkeypatch.setattr(ftm.Context, "memory_seq_rm", flaky_rm)
    control = MetalEngine(model, plain_config())
    rid_b = run(cached, PROMPT_B)
    rid_c = run(control, PROMPT_B)
    assert cached.tokens_of(rid_b) == control.tokens_of(rid_c)
